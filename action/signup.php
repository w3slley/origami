<?php
	...
	$user->setUserSignUp($first_name, $last_name, $user_name, $email, $hashedPassword); //setting user data to class
	...
	elseif(!empty($user_name) OR !empty($email)){
		...
		if(!empty($user_name)){ //If the username has some value, it goes to the DB and checks to see whether there are any other user with the same username.
			if($user->checkUsername($user_name)){ //If it does, then it returns this p tag used in the js file and gives the username control variable the value of one, preventing the user to signup.
